    ...
    for filename in pattern:
        ...
        with fits.open(filename, 'update') as hdul:
            ...
            if func == "update_keyword":                
                ...
                valore = t.mjd # Perfetto. Ora ho key, valore e filename

            if key in hdr:
                print(filename, key,":", hdr[key], "--->", str(valore))
            else:
                print(filename, key,":", "undefined --> ", str(valore))
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 5 :  # A te una gestione migliore del numero di argomenti!
        print(" Usage:  "+sys.argv[0]+" Provide function, KEYWORD, value, <list of FITS files>")
        sys.exit()
